...
export function useScrollNavigation(isAdminPage, opened) {
  ...
  const scrollToNext = useCallback(() => {
    ...
    const nextSection = sections.find(sec => {
      const absoluteTop = sec.getBoundingClientRect().top + currentScroll;
      return absoluteTop > currentScroll + OFFSET + BUFFER; 
    });
    
    if (nextSection) {
      const topPosition = (nextSection.getBoundingClientRect().top || 0) + window.scrollY - OFFSET;
      if (!isNaN(topPosition)) window.scrollTo({ top: topPosition, behavior: 'smooth' });
    } else if (sections.length > 0) {
      const topPosition = (sections[sections.length - 1].getBoundingClientRect().top || 0) + window.scrollY - OFFSET;
      if (!isNaN(topPosition)) window.scrollTo({ top: topPosition, behavior: 'smooth' });
    }
  }, []);
  ...
  const scrollToPrev = useCallback(() => {
    ...
    const prevSection = [...sections].reverse().find(sec => {
      const absoluteTop = sec.getBoundingClientRect().top + currentScroll;
      return absoluteTop < currentScroll - BUFFER; 
    });

    if (prevSection) {
      const topPosition = (prevSection.getBoundingClientRect().top || 0) + window.scrollY - OFFSET;
